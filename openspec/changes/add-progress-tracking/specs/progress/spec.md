...
waist, hips, arm, and thigh circumferences — plus an optional note. The date SHALL
default to the current time when not provided. An entry SHALL be rejected when it
contains no metric value, and any provided metric SHALL be a non-negative number.
Recording SHALL require an authenticated session and SHALL fail with a not-found
error for a non-existent client.
...
- **WHEN** the trainer submits an entry with no metric value
